...
const Sidebar = ({ local_varaiable, ThemeChanger }) => {
  ...
  useEffect(() => {
    // keep small history stack of pathnames
    ...
    if (mainContent) mainContent.addEventListener("click", mainContentClickFn);
  ...
  //  In Horizontal When we click the body it should be Closed
  ...
  function setSidemenu(list) {
    let dd = list ? list.path + "" : router.pathname;
    if (menuitems) {
    ...
    if (localStorage.getItem("ynexverticalstyles") == "icontext") {
      // keep placeholder if you want behavior here
    }
  ...
  function Onhover() {
    ...
    if (
      (theme.dataToggled == "icon-overlay-close" ||
        theme.dataVerticalStyle == "detached") &&
    ...
    const resizeHandler = () => {
      ...
      setTimeout(() => {
        if (window.innerWidth <= 992) {
          if (mc) {
            mc.addEventListener("click", menuClose);
            menuClose();
          }
        } else {
          if (mc) {
            mc.removeEventListener("click", menuClose);
            menuClose();
          }
        }
      }, 100);
  ...
  useEffect(() => {
    ...
    return () => {
      if (typeof window !== "undefined") {
        window.removeEventListener("scroll", pages);
      }
    };
  }, []);

  // handleClick: accepts either (event, href) OR a single string href
  const handleClick = (eOrHref, maybeHref) => {
    let hrefToPush = typeof eOrHref === "string" ? eOrHref : maybeHref;
    // if no href, nothing to do
    if (!hrefToPush) return;
    // prevent default only if it's an event
    if (eOrHref && typeof eOrHref.preventDefault === "function") {
        ...
        <div className="main-sidebar-header">
          <Link href="/components/dashboards/crm/" className="header-logo">
            <img
              ...
              }/assets/images/brand-logos/evezonbgremoved.png`}
              alt="logo"
          ...
          <div className="text-white">Evezon India</div>
        </div>
                ...
                <React.Fragment key={Math.random()}>
                  ...
                  {firstlayer.Items.map((secondlayer) =>
                    secondlayer.type === "link" ? (
                      <li
                        className={`slide ${secondlayer.active ? "active" : ""}`}
                        key={Math.random()}
                      >
                        <a
                          onClick={(_event) => {
                            ...
                            noChild();
                            handleClick("/"); // you pass a string href
                          }}
                          href={secondlayer.path + "/"}
                            ...
                            <React.Fragment key={Math.random()}>
                              {thirdlayer.type === "link" ? (
                                ...
                                >
                                  ...
                                  >
                                    {thirdlayer.icon}{" "}
                                    <span className="">{thirdlayer.title}</span>
                                        ...
                                        {fourthlayer.type === "link" ? (
                                          ...
                                          >
                                            <Link
                                              ...
                                              href={fourthlayer.path + ""}
                                              className={`side-menu__item ${
                                                fourthlayer.selected ? "active" : ""
                                              }`}
                                            >
                                              {fourthlayer.icon}
                                              {fourthlayer.title}
